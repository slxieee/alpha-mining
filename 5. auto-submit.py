...
# Version note: Auto-submit 6.0, Note: This is not an alpha generator, just an auxiliary submission tool
# Create command line argument parser, dates and max submissions can be adjusted as needed
...
# Read credentials from file (supports JSON format)
        ...
        if os.path.exists(file_path):
            with open(file_path, 'r') as file:
                ...
                try:
                    ...
                    # If not JSON format, try line-separated format
                    lines = content.split('\n')
                    if len(lines) >= 1:
...
# Read blacklist (create file if it doesn't exist)
...
# Update blacklist (real-time write)
...
# Unified login function (with retry mechanism)
def sign_in():
...
# Set Alpha properties (marking function)
